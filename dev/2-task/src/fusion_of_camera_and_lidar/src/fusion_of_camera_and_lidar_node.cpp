...
void cameraCallback(const sensor_msgs::ImageConstPtr &msg)
{
    ...
    // Declare a CvImagePtr to hold the converted image
    static cv_bridge::CvImagePtr cv_image;

    // Check the encoding type of the incoming image
    if (msg->encoding == sensor_msgs::image_encodings::RGB8) {
        ...
        cv::normalize(cv_image->image, depth_image_normalized, 0, 255, cv::NORM_MINMAX, CV_8UC1);

        // Show the normalized depth image
        //Image_Show(depth_image_normalized, origin_image);
    }
    else {
        // Handle unsupported image encoding
        ROS_ERROR("Unsupported image encoding: %s", msg->encoding.c_str());
        return; // Exit if encoding is unsupported
    }

    // Call the projection function after handling the image
    projection();
}
