...
int main( int argc, char **argv )
{
    ...
    std::unique_ptr<westonrobot::ScoutBase> EIE_base_ptr = std::make_unique<westonrobot::ScoutBase>();
    EIE_base_ptr->Connect( device_name, baud_rate );
    // EIE_base_ptr->EnableCommandedMode();     /* 使能控制模式, 已在SendRobotCmd中集成 */
    EIE_base EIE_base( std::move( EIE_base_ptr ), std::make_unique<ros::NodeHandle>( nh ) );
